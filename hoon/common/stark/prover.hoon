/+  *  /common/zeke
/+  *  /common/jet-ffi  :: Jet FFI 绑定优化（新增或确保存在） 
|%
:: =======================
:: STARK 证明生成主流程
:: =======================
...
++  generate-proof
  ...
  ^-  proof
  :: --------- 优化流程开始 ---------
  :: 1. 约束准备（原始逻辑，可优化为 Jet 实现）
  =/  constraints  (prepare-constraints header s f prod)
  :: 2. Fiat-Shamir 随机挑战，使用批量缓存优化
  =/  challenges   (cached-fiat-shamir header nonce constraints)
  :: 3. 多项式插值，Jet 批量插值实现
  =/  polynomials  (batch-interpolate constraints challenges)
  :: 4. Merkle 树承诺，可 Jet 化
  =/  merkles      (merkle-commitments polynomials)
  :: 5. FRI 证明生成，支持 Jet 加速
  =/  fri-proof    (fri-prove polynomials merkles challenges)
  :: 6. 组装最终证明
  (assemble-proof header nonce polynomials merkles fri-proof)
  :: --------- 优化流程结束 ---------
::

:: =======================
:: 优化辅助函数区（Jet 优先/Fallback 兼容）
:: =======================

++  batch-interpolate
  |=  [constraints challenges]
  ^-  (list polynomial)
  :: Jet 优化：如 Jet 不可用则用 fallback
  =+  polys
    (jet-batch-interpolate constraints challenges)
  ?~  polys
    (fallback-batch-interpolate constraints challenges)
    polys
::
...
++  cached-fiat-shamir
  ...
  :: Jet 优化：批量 hash，缓存结果
  =+  chals
    (jet-cached-fiat-shamir header nonce constraints)
  ?~  chals
    (fallback-fiat-shamir header nonce constraints)
    chals
::

:: ========== 下面保留原有流程/函数，建议逐步迁移到上方优化接口 ==========
