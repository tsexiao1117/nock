...
++  verify
  ...
  :: --------- 优化流程开始 ---------
  :: 1. 约束与多项式提取
  =/  constraints  (extract-constraints proof)
  =/  polynomials  (extract-polynomials proof)
  =/  merkles      (extract-merkles proof)
  :: 2. 随机挑战准备（批量缓存）
  =/  challenges   (cached-fiat-shamir header nonce constraints)
  :: 3. 批量 FRI 验证，Jet 优化
  =/  fri-ok       (batch-fri-verify polynomials merkles challenges)
  :: 4. 批量 Merkle 校验，Jet 优化
  =/  merkle-ok    (batch-verify-merkles merkles)
  :: 5. 并发执行所有校验
  ?&  [fri-ok merkle-ok]
    %.y
    %.n
...
++  batch-fri-verify
  ...
  ^-  ?
  :: Jet 优化：批量 FRI 校验，支持多核并行
  ?:  (jet-batch-fri-verify polynomials merkles challenges)
    %.y
    (fallback-batch-fri-verify polynomials merkles challenges)
::

++  batch-verify-merkles
  |=  merkles
  ^-  ?
  :: Jet 优化：批量 Merkle 校验
  ?:  (jet-batch-verify-merkles merkles)
    %.y
    (fallback-batch-verify-merkles merkles)
::
